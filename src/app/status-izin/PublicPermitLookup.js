        ...
        <article className={`relative overflow-hidden rounded-[2rem] border bg-slate-900 p-6 shadow-2xl sm:p-8 ${active ? 'border-emerald-400/50 shadow-emerald-950' : 'border-rose-500/60 shadow-rose-950'}`}>
          ...
          <div className="relative">
            <div className="flex items-start justify-between gap-4">
              <div><p className="text-[10px] font-bold uppercase tracking-[0.28em] text-slate-400">Digital Market Permit</p><h2 className="mt-2 text-xl font-black">{permit.namaPasar}</h2></div>
            ...
            <div className="my-8 grid grid-cols-[1fr_auto] items-end gap-6 border-y border-slate-800 py-7">
              <div><p className="text-xs uppercase tracking-wider text-slate-400">Nomor Ruang</p><p className="mt-1 font-mono text-4xl font-black tracking-tight text-white">{permit.nomorRuang}</p></div>
            ...
            <dl className="grid gap-5 sm:grid-cols-2"><div><dt className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Nomor Kartu</dt><dd className="mt-1 font-mono text-sm text-slate-200">{permit.nomorKartu}</dd></div><div><dt className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Tanggal Terbit</dt><dd className="mt-1 text-sm text-slate-200">{formatDate(permit.tanggalTerbit)}</dd></div><div className="sm:col-span-2"><dt className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Berlaku Sampai</dt><dd className={`mt-1 text-lg font-bold ${active ? 'text-emerald-300' : 'text-rose-300'}`}>{formatDate(permit.tanggalKedaluwarsa)}</dd></div></dl>
